  ...
  export default function AdminDashboard() {
    ...
    const handleMaterialTypeChange = (text) => {
      ...
      if (!text.trim()) {
        setMaterialTypeError("Material type cannot be empty.");
      } else {
    ...
    const handleAddMaterial = async () => {
      ...
      try {
        ...
        const isDuplicate = materialHistory.some(
          (item) =>
            item.name?.trim().toLowerCase() === trimmedName.toLowerCase() &&
            item.type?.trim().toLowerCase() === trimmedType.toLowerCase()
        );

        if (isDuplicate) {
          setResultType("error");
          setResultMessage(`"${trimmedName}" with type "${trimmedType}" already exists.`);
          setIsLoading(false);
        ...
        setResultMessage(
          `"${trimmedName}" has been successfully added as a material.`,
        );
        setNewMaterialName("");
        setNewMaterialType("");
      } catch (err) {
    ...
    const handleAddOrigin = async () => {
      const trimmedOrigin = newOriginName.trim();

      if (!trimmedOrigin) {
        setOriginNameError("Origin name cannot be empty.");
        return;
      } else if (trimmedOrigin.length < 3) {
        setOriginNameError("Origin name must be at least 3 characters.");
        return;
      }
      ...
      try {
        ...
        const isDuplicate = originHistory.some(
          (item) => item.name?.trim().toLowerCase() === trimmedOrigin.toLowerCase()
        );

        if (isDuplicate) {
          setResultType("error");
          setResultMessage(`"${trimmedOrigin}" already exists in the origins record.`);
          setIsLoading(false);
          setResultVisible(true);
          return;
        }
        ...
        setResultType("success");
        setResultMessage(
          `"${trimmedOrigin}" has been successfully added as an origin.`,
        );
        setNewOriginName("");
      } catch (err) {
        const serverMessage =
        ...
        <Modal visible={showAddMaterialModal} transparent animationType="fade">
          <View style={styles.addMaterial_modalOverlay}>
            <View style={styles.addMaterial_modalContainer}>
              ...
              {/* Input */}
              <View style={[styles.addMaterial_modalInputWrap, styles.inputMarginTop, 
                materialNameError ? styles.inputErrorBorder : null]}>
                <TextInput
                  ...
                  onChangeText={handleMaterialNameChange}
                />
              </View>
              {materialNameError ? <Text style={styles.errorText}>{materialNameError}</Text> : null}
              <View style={[styles.addMaterial_modalInputWrap, 
                styles.inputMarginTop, 
                materialTypeError ? styles.inputErrorBorder : null
              ]}>
                <TextInput
                  ...
                  onChangeText={handleMaterialTypeChange}
                />
              </View>
              {materialTypeError ? <Text style={styles.errorText}>{materialTypeError}</Text> : null}
        ...
        <Modal visible={showAddOriginModal} transparent animationType="fade">
          <View style={styles.addMaterial_modalOverlay}>
            <View style={styles.addMaterial_modalContainer}>
              ...
              {/* Input */}
              <View style={[styles.addMaterial_modalInputWrap,
                originNameError ? styles.inputErrorBorder : null
              ]}>
                <TextInput
                  ...
                  value={newOriginName}
                  maxLength={64}
                  onChangeText={handleOriginNameChange}
                />
              </View>
              {originNameError ? <Text style={styles.errorText}>{originNameError}</Text> : null}
              {/* Divider */}
              <View style={styles.addMaterial_modalDivider} />
